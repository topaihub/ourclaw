...
- [x] **M2-09 收口 retrieval / embeddings / memory ranking**
  ...
  - 本轮实现（2026-03-13，第一子步）：
    - `src/domain/memory_runtime.zig` 已把 retrieval 命中结构从单一 `score` 扩展为 `rank / ts_unix_ms / embedding_strategy / ranking_reason / embedding_score / keyword_overlap / kind_weight`
    - memory ranking 已从纯 `dotProduct` 暴露为更清晰的本地 embedding + lexical overlap + kind weight 组合评分，形成第一版可解释 ranking metadata
    - `src/commands/memory_retrieve.zig` 已把 richer ranking 字段稳定输出到命令返回 JSON
    - `src/config/field_registry.zig`、`src/runtime/config_runtime_hooks.zig`、`src/runtime/app_context.zig` 已补 `memory.embedding_provider` / `memory.embedding_model` 运行时接线，形成 `config.set -> runtime hook -> memory runtime` 最小闭环
    - `tests/smoke.zig` 与 `src/domain/memory_runtime.zig` 单测已补 retrieval rich output / ranking metadata 断言
    - smoke 已补 memory embedding config 变更后 retrieval 输出回显 provider/model 的断言
  - 本轮实现（2026-03-16，第二子步）：
    - `src/providers/root.zig` 已新增 `EmbeddingProvider` 抽象，`ProviderRegistry` 现可投影为 provider-backed embeddings 接口，而不是让 `memory_runtime` 直接依赖具体 registry 细节
    - `src/domain/memory_runtime.zig` 已改为优先消费 embedding provider 抽象，并在 provider 不可用或调用失败时保留本地 fallback
    - `src/providers/openai_compatible.zig` 现有 embeddings 入口已通过 provider abstraction 路径被真实消费，`tests/smoke.zig` 与 `src/domain/memory_runtime.zig` 单测已验证 `provider_proxy_v1` 输出语义
    - 顺手修正 `memory_runtime.zig` 中 `embedQuery` / `buildEmbedding` 的编译漂移，恢复 memory 相关命令与测试可编译状态
    - 验证：`zig build test --summary all` 通过（123/123）

- [x] **M2-10 把现有 provider/tool 做到生产语义第一阶段**
  ...
  - 本轮实现（2026-03-16）：
    - `src/providers/root.zig` 已扩展 `ProviderRequest` 的 `timeout_secs / retry_budget`，并为 provider 错误补 `ProviderErrorInfo` / `mapError()` 映射
    - `src/providers/openai_compatible.zig` 已消费 `timeout_secs`，并补 mock timeout / retry-once 语义，用于回归 provider 超时与重试路径
    - `src/tools/root.zig` 已补 `ToolInvokePolicy`，高风险工具默认需要显式 `confirm_risk`，并新增 `TOOL_BUDGET_EXCEEDED` / `TOOL_RISK_CONFIRMATION_REQUIRED` 错误映射
    - `src/domain/tool_orchestrator.zig` 已补 `remaining_budget / confirm_risk` 执行约束，并新增 `tool.call.audit` 事件，稳定回写 started / denied / finished 等审计语义
    - `src/domain/agent_runtime.zig`、`src/commands/agent_run.zig`、`src/commands/agent_stream.zig` 已把 `provider_timeout_secs / provider_retry_budget / tool_call_budget / confirm_tool_risk` 接入实际运行链路
    - `tests/smoke.zig`、`src/providers/root.zig`、`src/providers/openai_compatible.zig`、`src/tools/root.zig`、`src/domain/tool_orchestrator.zig`、`src/domain/agent_runtime.zig` 已补 retry / timeout / denied / budget 回归覆盖
