...
pub fn parseArgs(allocator: std.mem.Allocator, args: []const []const u8) anyerror!OwnedRequest {
    ...
    if (std.mem.eql(u8, args[0], "metrics.summary") or std.mem.eql(u8, args[0], "gateway.status") or std.mem.eql(u8, args[0], "gateway.auth.status") or std.mem.eql(u8, args[0], "service.status") or std.mem.eql(u8, args[0], "skills.list") or std.mem.eql(u8, args[0], "cron.list") or std.mem.eql(u8, args[0], "heartbeat.status") or std.mem.eql(u8, args[0], "tunnel.status") or std.mem.eql(u8, args[0], "mcp.list") or std.mem.eql(u8, args[0], "hardware.list") or std.mem.eql(u8, args[0], "voice.status") or std.mem.eql(u8, args[0], "node.list") or std.mem.eql(u8, args[0], "devices.list")) {
        const params = try allocator.alloc(framework.ValidationField, 0);
        return .{
