...
const budget = @import("agent_runtime_budget.zig");
const memory_runtime = @import("memory_runtime.zig");
const provider_loop = @import("agent_runtime_provider_loop.zig");
...
pub const AgentRuntime = struct {
    ...
    const Self = @This();
    const ToolRoundSource = provider_loop.ToolRoundSource;
    ...
    pub fn runStream(self: *Self, request: AgentRunRequest) anyerror!AgentStreamResult {
        ...
        try budget.ensureNotCancelled(request);
        ...
        if (request.tool_id) |tool_id| {
            try budget.ensureNotCancelled(request);
            try provider_loop.executeToolRound(
                .{
        ...
        while (true) {
            try budget.ensureExecutionBudget(request, started_at, provider_rounds, remaining_tool_budget);
            provider_rounds += 1;
            try budget.ensureNotCancelled(request);
            ...
            var assembled = try prompt_assembly.build(self.allocator, .{
                ...
                .provider_round_budget = request.provider_round_budget,
                .provider_attempt_budget = request.provider_attempt_budget,
                .provider_retry_budget = request.provider_retry_budget,
        ...
        try turn_writer.print("{{\"providerId\":\"{s}\",\"model\":\"{s}\",\"allowProviderTools\":{s},\"promptProfile\":\"{s}\",\"responseMode\":\"{s}\",\"providerRounds\":{d},\"providerRoundBudget\":{d},\"providerRoundsRemaining\":{d},\"providerAttemptBudget\":{d},\"providerAttemptsRemaining\":{d},\"toolId\":", .{ request.provider_id, selected_model, if (last_provider_tools_enabled) "true" else "false", @tagName(request.prompt_profile), @tagName(request.response_mode), provider_rounds, request.provider_round_budget, budget.remainingProviderRounds(request.provider_round_budget, provider_rounds), request.provider_attempt_budget, remaining_provider_attempt_budget });
