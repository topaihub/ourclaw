...
fn handle(ctx: *const framework.CommandContext) anyerror![]const u8 {
    ...
    const decision = app.pairing_registry.revokeToken(id);
    return std.fmt.allocPrint(ctx.allocator, "{{\"id\":\"{s}\",\"changed\":{s},\"state\":\"{s}\",\"token\":null}}", .{ id, if (decision.changed) "true" else "false", decision.state.asText() });
}
