...
pub fn definition(command_services: *services_model.CommandServices) framework.CommandDefinition {
    return .{
        .id = "device.token.rotate",
        .method = "device.token.rotate",
        .description = "Rotate a paired device token",
        .authority = .operator,
...
fn handle(ctx: *const framework.CommandContext) anyerror![]const u8 {
    ...
    const decision = app.pairing_registry.rotateToken(id);
    var buf: std.ArrayListUnmanaged(u8) = .empty;
    defer buf.deinit(ctx.allocator);
    const writer = buf.writer(ctx.allocator);
    try writer.print("{{\"id\":\"{s}\",\"changed\":{s},\"state\":\"{s}\",\"token\":", .{ id, if (decision.changed) "true" else "false", decision.state.asText() });
