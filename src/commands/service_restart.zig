...
fn handle(ctx: *const framework.CommandContext) anyerror![]const u8 {
    ...
    app.service_manager.restart();
    const status = app.service_manager.status();
    return std.fmt.allocPrint(ctx.allocator, "{{\"restarted\":true,\"runtimeRunning\":{s},\"restartCount\":{d},\"stopCount\":{d}}}", .{ if (status.runtime_running) "true" else "false", status.restart_count, status.stop_count });
}
