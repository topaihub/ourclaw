...
fn handle(ctx: *const framework.CommandContext) anyerror![]const u8 {
    ...
    const extra = try std.fmt.allocPrint(ctx.allocator, ",\"action\":\"restart\",\"stopApplied\":{s},\"startApplied\":{s}", .{ if (restart.stop_changed) "true" else "false", if (restart.start_changed) "true" else "false" });
