...
fn handle(ctx: *const framework.CommandContext) anyerror![]const u8 {
    ...
    app.service_manager.stop();
    const status = app.service_manager.status();
    return std.fmt.allocPrint(ctx.allocator, "{{\"stopped\":true,\"runtimeRunning\":{s},\"stopCount\":{d}}}", .{ if (status.runtime_running) "true" else "false", status.stop_count });
}
