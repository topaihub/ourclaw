...
fn handle(ctx: *const framework.CommandContext) anyerror![]const u8 {
    ...
    return std.fmt.allocPrint(ctx.allocator, "{{\"stopped\":true,\"runtimeRunning\":{s},\"stopCount\":{d},\"changed\":{s}}}", .{ if (status.runtime_running) "true" else "false", status.stop_count, if (changed) "true" else "false" });
}
